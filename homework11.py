...
"""
Task: Creați o funcție cu numele `task_5` care primește un număr variabil de argumente 
și returnează două liste separate.
Prima listă conține toate argumentele de tip întreg sortate în ordine crescătoare, 
iar a doua listă conține denumirea tuturor argumentelor keyword care sunt de tip string sortate în ordine alfabetică.
...
def task_5(*args,**kwargs):
    ...
    strings = sorted([key for key, value in kwargs.items() if isinstance(value, str)])
...
"""
Task: Creați o funcție cu numele `task_10` care primește un 
număr variabil de argumente de tip integer și un argument `number` de tip integer.
...
"""
Task: Creați o funcție cu numele `task_14` 
care primește un argument `sub_string` de tip string și un număr variabil de 
argumente de tip string.
...
def task_14(*args, sub_string):
    return [arg for arg in args if sub_string in arg.lower()]
# CODUL TĂU VINE MAI SUS:
    ...
    def is_prime(n):
        ...
        for i in range(2, int(n**0.5) + 1):
            if n % i == 0:
                return False
        return True
    prime_args = [arg for arg in args if is_prime(arg)]
    result = {}
    for arg in prime_args:
        if arg in result:
            result[arg] += 1
        else:
            result[arg] = 1
